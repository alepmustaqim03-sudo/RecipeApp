    ...
    <View style={s.container}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        {recipe.ingredients.map(i => (
          <Text key={i.id} style={s.item}>• {i.text}</Text>
        ))}

        <Text style={s.section}>Steps</Text>
        {recipe.steps.map(st => (
          <Text key={st.id} style={s.item}>• {st.text}</Text>
        ))}
      ...
      >
        ...
        <View style={s.confirmCard}>
          ...
          <Text style={s.confirmMsg}>
            This action can’t be undone. “{recipe.name}” will be removed from your list.
          </Text>
          <View style={s.confirmRow}>
            <TouchableOpacity style={[s.btnSm, s.btnNeutral]} onPress={() => setShowDelete(false)}>
              <Text style={s.btnNeutralTxt}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[s.btnSm, s.btnDanger]} onPress={confirmDelete}>
...
const s = StyleSheet.create({
  ...
  // modal styles (matching your other sheets)
